...
/**
 * Created by ims13 on 7/22/16.
 *
 * http://stackoverflow.com/questions/27614439/how-to-detect-my-screen-orientation-in-portrait-locked-screen-in-android
 */
public abstract class SimpleOrientationListener extends OrientationEventListener {
    ...
    private void reportOrientationChanged(final int currentOrientation) {
        ...
        if (currentOrientation == Surface.ROTATION_0)           orientation = ActivityInfo.SCREEN_ORIENTATION_PORTRAIT;
        else if (currentOrientation == Surface.ROTATION_90)     orientation = ActivityInfo.SCREEN_ORIENTATION_REVERSE_LANDSCAPE;
        else if (currentOrientation == Surface.ROTATION_180)    orientation = ActivityInfo.SCREEN_ORIENTATION_REVERSE_PORTRAIT;
        else if (currentOrientation == Surface.ROTATION_270)    orientation = ActivityInfo.SCREEN_ORIENTATION_LANDSCAPE;

        onSimple4OrientationChanged(orientation);
    }
     ...
     * @param orientation
     */
    public abstract void onSimple4OrientationChanged(int orientation);
